	...
	<section id="reservasi" class="section-padding">
		...
		<div class="row">
			<div class="col-sm-5">
				...
				<form action="/reservasi/store" method="post" style="color:#282B2F"> 
				{{ csrf_field() }}
				<h3>Tambah Data reservasi</h3>
					...
					<div class="form-row">
						...
						<label for="id_pasien">Pilih ID Pasien</label>
								<select name="id_pasien" class="form-control" id="id_pasien" required="required">
										<option selected value="" disable selected></option>
										@foreach($pasien as $ps)
										<option value="{{$ps->id_pasien}}">{{$ps->id_pasien}}</option>
										@endforeach
								</select>
                        </div>
						<div class="form-group col-sm-12">
						<label for="id_petugas">Pilih ID Petugas</label>
							<select name="id_petugas" class="form-control" id="id_petugas" required="required">
							<option selected value="" disable selected></option>
							@foreach($petugas as $pg)
							<option value="{{$pg->id_petugas}}">{{$pg->id_petugas}}</option>
							@endforeach
							</select>
						</div>	
						<div class="form-group col-sm-12">
